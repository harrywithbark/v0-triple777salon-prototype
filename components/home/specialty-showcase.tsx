    ...
    <section ref={sectionRef} className="py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Image */}
          <div className="relative h-[400px] lg:h-[500px] rounded-2xl overflow-hidden scroll-fade-up card-hover">
            <img
          ...
          {/* Content */}
          <div className="space-y-8 scroll-fade-up stagger-2">
            <div>
              <p className="text-[#E8A598] text-sm font-medium tracking-widest uppercase mb-4">
                Technical Excellence
              </p>
              <h2 className="font-serif text-3xl sm:text-4xl font-semibold text-[#0D0809] mb-4 text-balance">
              ...
              {specialties.map((specialty, index) => (
                <li key={index} className="flex items-center gap-3">
                  <div className="w-6 h-6 rounded-full bg-[#E8A598]/20 flex items-center justify-center shrink-0">
                    <Check className="w-3.5 h-3.5 text-[#D4324B]" />
                  </div>
                  <span className="text-[#0D0809] font-medium">{specialty}</span>
                </li>
              ))}
            </ul>
          </div>
